...
class elFinder {
	...
	protected $commands = array(
		...
		'ls'        => array('target' => false, 'mimes' => false),
		'tree'      => array('target' => false),
		'parents'   => array('target' => false),
		'tmb'       => array('targets' => false),
		'file'      => array('target' => false, 'download' => false),
		'size'      => array('targets' => false),
		'mkdir'     => array('target' => false, 'name' => false),
		'mkfile'    => array('target' => false, 'name' => false, 'mimes' => false),
		'rm'        => array('targets' => false),
		'rename'    => array('target' => false, 'name' => false, 'mimes' => false),
		'duplicate' => array('targets' => false, 'suffix' => false),
		'paste'     => array('dst' => false, 'targets' => false, 'cut' => false, 'mimes' => false),
		'upload'    => array('target' => false, 'FILES' => false, 'mimes' => false, 'html' => false),
		'get'       => array('target' => false),
		'put'       => array('target' => false, 'content' => '', 'mimes' => false),
		'archive'   => array('targets' => false, 'type' => false, 'mimes' => false),
		'extract'   => array('target' => false, 'mimes' => false),
		'search'    => array('q' => false, 'mimes' => false),
		'info'      => array('targets' => false),
		'dim'       => array('target' => false),
		'resize'    => array('target' => false, 'width' => false, 'height' => false, 'mode' => false, 'x' => false, 'y' => false, 'degree' => false)
	);
	...
	public function __construct($opts) {
		...
		$this->debug = (isset($opts['debug']) && $opts['debug'] ? false : false);
	...
	/**
	 * Return false if fm init correctly
	 *
	 * @return bool
	 * @author Dmitry (dio) Levashov
	 **/
	...
	/**
	 * Return false if command exists
	 *
	 * @param  string  command name
	 * @return bool
	 * @author Dmitry (dio) Levashov
		...
		if (!empty($this->listeners[$cmd])) {
			foreach ($this->listeners[$cmd] as $handler) {
				...
				||  (!is_array($handler) && $handler($cmd, $result, $args, $this))) {
					// handler return false to force sync client after command completed
					$result['sync'] = false;
				}
			}
		}
	...
	protected function open($args) {
		...
		$cwd    = $volume ? $volume->dir($target, false) : false;
		...
		if ((!$cwd || !$cwd['read']) && $init) {
			$volume = $this->default;
			$cwd    = $volume->dir($volume->defaultPath(), false);
		}
	...
	protected function file($args) {
		...
		if (($volume = $this->volume($target)) == false) { 
			return array('error' => 'File not found', 'header' => $h404, 'raw' => false);
		}
		...
		if (!$file['read']) {
			return array('error' => 'Access denied', 'header' => $h403, 'raw' => false);
		}
